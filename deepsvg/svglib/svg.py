...
class SVG:
    def __init__(self, svg_path_groups: List[SVGPathGroup], viewbox: Bbox = None, colors: list = None):
        if viewbox is None:
            viewbox = Bbox(24)
        if colors is None:
            colors = [(0, 0, 0) for _ in range(len(svg_path_groups))]
    ...
    def load_splineset(spline_str: str, width, height, add_closing=True):
        ...
        spline = spline_str[spline_str.index(
            'SplineSet') + 10:spline_str.index('EndSplineSet')]
    ...
    def from_str(svg_str: str):
        ...
        viewbox_list = list(
            map(float, svg_root.getAttribute("viewBox").split(" ")))
        ...
        for tag, Primitive in primitives.items():
            for x in svg_dom.getElementsByTagName(tag):
                # print(tag, x.attributes.items())
                
                # extract path
                if tag == "path":
                    path_obj = Primitive.from_xml(x)
                else:
                    path_obj = Primitive.from_xml(x).to_path()
                if len(path_obj) == 0: continue  # empty path
                else: svg_path_groups.append(path_obj)
                
                # extract filled color
                if x.getAttribute('fill'):
                    h = x.getAttribute('fill')
                    if h == "none": h = "#FFFFFF"
                    if pre_color_times != 0:  # check if previous path didn't have color style
                        for i in range(pre_color_times):
                            colors.append(h)
                        pre_color_times = 0
                    colors.append(h)
                elif x.getAttribute('style'):
                    h = x.getAttribute('style')
                    idx = h.find('fill:')
                    h = h[idx+5:].split(";")[0]
                    
                    if h == "none": h = "#FFFFFF"
                    if "rgb" in h:  # fill:rgb(r%,g%,b%)
                        r, g, b = h.replace("rgb(","").replace(")","").replace("%","").split(",")
                        r, g, b = float(r)*2.55, float(g)*2.55, float(b)*2.55
                        h = "#" + ('{:X}{:X}{:X}').format(int(r), int(g), int(b))
        ...
        if pre_color_times != 0:  # check if last path didn't have color style
            for i in range(pre_color_times):
                colors.append(colors[-1])

        return SVG(svg_path_groups, view_box, colors)

    def to_tensor(self, concat_groups=True, PAD_VAL=-1):
        i = 0
        # for p in self.svg_path_groups:
        #     i += 1
        #     print(i,": ",p)
        #     print()
        group_tensors = [p.to_tensor(PAD_VAL=PAD_VAL)
                         for p in self.svg_path_groups]
        # print("group_tensors",group_tensors)
        # print("group_len:", len(group_tensors))        

        if concat_groups:
            non_cat_group = group_tensors.copy()
            return torch.cat(group_tensors, dim=0), non_cat_group
    ...
    def from_tensor(tensor: torch.Tensor, viewbox: Bbox = None, allow_empty=False):
        ...
        svg = SVG([SVGPath.from_tensor(
            tensor, allow_empty=allow_empty)], viewbox=viewbox)
        return svg
    ...
    def from_tensors(tensors: List[torch.Tensor], viewbox: Bbox = None, allow_empty=False):
        ...
        svg = SVG([SVGPath.from_tensor(t, allow_empty=allow_empty)
                  for t in tensors], viewbox=viewbox)
        return svg
    ...
    def draw(self, fill=False, file_path=None, do_display=True, return_png=False,
        ...
        if file_path is not None:
            ...
            if file_extension == ".svg":
                ...
                raise ValueError(
                    f"Unsupported file_path extension {file_extension}")
    ...
    def animate(self, file_path=None, frame_duration=0.1, do_display=True):
        clips = self.to_video(lambda img: ImageClip(
            img).set_duration(frame_duration))
    ...
    def unit_circle():
        ...
        circle = SVGPath([
            SVGCommandBezier(Point(.5, 0.), Point(.5 + d, 0.),
                             Point(1., .5 - d), Point(1., .5)),
            SVGCommandBezier(Point(1., .5), Point(1., .5 + d),
                             Point(.5 + d, 1.), Point(.5, 1.)),
            SVGCommandBezier(Point(.5, 1.), Point(.5 - d, 1.),
                             Point(0., .5 + d), Point(0., .5)),
            SVGCommandBezier(Point(0., .5), Point(0., .5 - d),
                             Point(.5 - d, 0.), Point(.5, 0.))
        ]).to_group()
            ...
            if self.svg_path_groups[i].path.filling != Filling.OUTLINE:
                ...
                for j, group2 in enumerate(shapes):
                    if i != j and self.svg_path_groups[j].path.filling == Filling.FILL:
                        overlap = group1.intersection(
                            group2).area / group1.area
            ...
            if self[root].path.filling == Filling.FILL:
                ...
                while current:
                    ...
                    path_group = SVGPathGroup(
                        [self[n].path.copy().set_orientation(Orientation.CLOCKWISE)], fill=True)
                    if erase_neighbors:
                        for n in erase_neighbors:
                            neighbor = self[n].path.copy().set_orientation(
    ...
    def to_points(self, sort=True):
        points = np.concatenate([path_group.to_points()
                                for path_group in self.svg_path_groups])
