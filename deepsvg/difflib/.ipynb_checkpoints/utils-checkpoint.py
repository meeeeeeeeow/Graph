...
def plot_points(p, point_color, viewbox=None, show_color=False, show_colorbar=False, image_file=None, return_img=False):
    ...
    if viewbox is not None:
        set_viewbox(viewbox)
    c_list = ['Greys', 'Purples', 'Blues', 'Greens', 'Oranges', 'Reds',
              'YlOrBr', 'YlOrRd', 'OrRd', 'PuRd', 'RdPu', 'BuPu',
              'GnBu', 'PuBu', 'YlGnBu', 'PuBuGn', 'BuGn', 'YlGn']
    # kwargs = {"c": range(len(p)), "cmap": cm} if show_color else {}
    # plt.scatter(p[:, 0], p[:, 1], **kwargs)
    # colors = cm(np.linspace(0, 1, len(nearest_points)))
    # label the path order

    for i in range(1, len(p), 9):
        plt.scatter(p[i-1:i+8, 0], p[i-1:i+8, 1], s=5, color=point_color)#, cmap=random.choice(c_list))

    # for i in range(1, len(p), 9):
    #     n_point_index = nearest_points[i//9]
    #     plt.scatter(p[i-1+n_point_index, 0],
    #                 p[i-1+n_point_index, 1], c="yellow")
    #     plt.annotate(i//9+1, (p[i+3, 0], p[i+3, 1]), fontsize=5)
